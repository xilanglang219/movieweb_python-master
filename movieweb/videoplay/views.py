...
def play(request, id):
    ...
    c = {"user": sign, "video_source": video_source, "video_id": video_id, "movie_payment_obj": len(movie_payment_obj),
         "pay": pay, "url": url, "comments_list": comments_list}
    ...
    if request.method == 'POST':
        ...
        if uf_login.is_valid():
            ...
            try:
                ...
                return render_to_response(request, 'user_login.html', {"userName_error": True})
            # 获取数据库(模型)中对应的对象
            ...
            # 输入的密码不匹配数据库中的密码 执行
                ...
                return render_to_response(request, 'user_login.html', {"userPassword_error" : True})

        # 如果输入的内容不合法（数据为空）
        else:
            return render_to_response(request, 'user_login.html', {'userForm_error': True})
    else:
        uf_login = UserForm()
    # 加载用户登录模板
